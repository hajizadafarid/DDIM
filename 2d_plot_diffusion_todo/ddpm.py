...
class DiffusionModule(nn.Module):
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        var = eta * torch.sqrt((1 - alpha_prod_t_prev) / (1 - alpha_prod_t)) * torch.sqrt(1 - alpha_prod_t/alpha_prod_t_prev)
        x0_given_t = 1/torch.sqrt(alpha_prod_t) * (xt - torch.sqrt(1 - alpha_prod_t) * self.network(xt, t))
        mu_tilda = torch.sqrt(alpha_prod_t_prev) * x0_given_t + torch.sqrt(1 - alpha_prod_t_prev - var**2) * (xt - torch.sqrt(alpha_prod_t) * x0_given_t) / torch.sqrt(1 - alpha_prod_t)
        z_t = torch.randn_like(xt)
        x_t_prev = mu_tilda + var * z_t
        ######################
        return x_t_prev
