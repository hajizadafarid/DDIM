    ...
    def forward(self, x, timestep, class_label=None):
        ...
        if self.use_cfg and class_label is not None:
            if self.training:
                ...
                size = class_label.size(0)

                index = torch.randperm(size)[:int(self.cfg_dropout * size)]
                class_label[index] = 0
                #######################
            ...
            cfgtemb = self.class_embedding(class_label)

            if temb.shape != cfgtemb.shape:
                temb = temb.repeat((cfgtemb.shape[0], 1))
